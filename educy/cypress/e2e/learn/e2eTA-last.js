...
describe('My first test', () => {
    ...
    it('Test QRIS Payment Method', {defaultCommandTimeout: 20000},() => {
        ...
        cy.get('[id="snap-midtrans"]').then($element=> {
            ...
            // Qris
            stripe = cy.wrap($body)
            stripe.find('[class="QRIS"]').then(instructionList => {
                cy.get(instructionList).find('[class="logo-border-payment-list"]').eq(0).then(qris => {
                    cy.get(qris).find('[class="logo-payment-type"]').click()
                    cy.wait(2000)
